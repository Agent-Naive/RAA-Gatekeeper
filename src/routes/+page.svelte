...
<div class="app-layout">
  ...
  <div class="main-container">
    ...
    <main class="content-pane">
      {#if isProcessing}<div class="loader-overlay"><div class="spinner"></div><p>Deep Scanning Archive...</p></div>{/if}
      <div class="view-wrapper">
        {#if activeTab === 'audit'}
          <h2>Audit Command Line</h2>
          <form class="tool-box" onsubmit={handleAudit}>
            <!-- svelte-ignore a11y_autofocus -->
            <input bind:value={commandInput} placeholder="e.g. ls *.txt" autocapitalize="none" autocorrect="off" spellcheck="false" autofocus />
        ...
        {:else if activeTab === 'archive'}
          ...
          <p>Scan inside ZIP repos without extracting to your system.</p>
...
<style>
  /* ... keeping all previous centered Flexbox and Dark Mode styles ... */
  ...
  @media (prefers-color-scheme: dark) { 
    .app-layout { background: #121212; } .content-pane { color: #eee; } .report-card { background: #1a1a1a; border-color: #333; } .verdict-text { color: #edf2f7; border-color: #333; } .reasoning-text { color: #cbd5e0; } .loader-overlay { background: rgba(18,18,18,0.9); } input { background: #222; border-color: #444; color: white; } 
  }
</style>
